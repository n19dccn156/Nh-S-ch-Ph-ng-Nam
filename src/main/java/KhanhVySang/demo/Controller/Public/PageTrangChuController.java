...
public class PageTrangChuController {
    ...
    @GetMapping(path = "/vi/quenmatkhau")
    public String quenMatKhau(){

        return "public/quenmatkhau";
    }
    ...
    public String sendEmail(ModelMap model, @RequestParam("email") String email) {
        try {
            ...
            String newPw = String.valueOf(clock.millis()%1000000);
            ...
            // update password            
            TaiKhoanModel taiKhoan =  taiKhoanService.findByEmail(email);
            taiKhoan.setMatKhau(newPw);
            int row = taiKhoanService.updateTaiKhoan(passwordEncoder.encode(newPw), email);
            if(row == 0)    return "redirect:/login";
